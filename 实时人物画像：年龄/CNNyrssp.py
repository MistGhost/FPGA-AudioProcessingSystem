...
def load_dataset(base_path, max_length=128):
    X = []
    y = []
    for label, category in enumerate(['old', 'young']):
...
# 设置数据路径
base_path = os.path.join(os.path.expanduser('~'), 'Desktop', '声音分类')
...
def predict_audio(file_path, max_length=128):
    ...
    if mel_spectrogram is not None:
        ...
        return 'old' if class_idx == 0 else 'young'
    else:
        return "Error loading audio."

# 测试单个音频文件
file_path = 'test4.mp3'  # 替换为实际的测试音频文件路径
